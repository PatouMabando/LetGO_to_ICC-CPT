...
export const login = async (req, res) => {
  try {
    ...
    const { delivered, dev } = await sendOtpSMS(phoneNumber, code);
    ...
    if (!delivered) {
      payload.message = "Failed to send OTP via SMS. Check your Twilio configuration.";
      console.error("Failed to send OTP via SMS. Check your Twilio configuration.");
    }
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    let JWT_SECRET = process.env.JWT_SECRET; // Get the secret here

    if (!JWT_SECRET) {
        console.error("JWT_SECRET is not defined in the environment variables.");
        JWT_SECRET = "fallbackSecret"; // Provide a fallback secret for development
        //return res.status(500).json({ error: "JWT_SECRET is not defined." }); // Comment out this line for development
    }
