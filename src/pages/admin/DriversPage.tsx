...
import { adminApi, type Driver, type Status } from "@/api/admin";
...
export default function DriversPage() {
  ...
  const isAvailable = (r: Driver) => r.availability === "available";
  ...
  const changeStatus = async (id: string, status: Status) => {
    ...
    try {
      ...
      adminApi.getDrivers(token).then(setRows);
    }
  };

  const toggleAvailability = async (id: string, available: boolean) => {
    const val = available ? "available" : "not_available";
    setRows((prev) =>
      prev.map((r) => (r.id === id ? { ...r, availability: val } : r)),
    );
    try {
      await adminApi.setDriverAvailability(token, id, val);
    } catch {
      adminApi.getDrivers(token).then(setRows);
    }
  };
                ...
                >
                  ...
                  <TableCell>
                    <Stack direction="row" alignItems="center" spacing={1}>
                      <Switch
                        checked={isAvailable(r)}
                        onChange={(e) =>
                          toggleAvailability(r.id, e.target.checked)
                        }
                        sx={{
                      ...
                      />
                      <Typography
                        sx={{ fontSize: 12, fontWeight: 900, color: "#64748B" }}
                      >
                        {isAvailable(r) ? "Available" : "Not available"}
                      </Typography>
                    </Stack>
                  </TableCell>
