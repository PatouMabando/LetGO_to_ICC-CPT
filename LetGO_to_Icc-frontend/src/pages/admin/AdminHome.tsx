...
export default function AdminHome() {
  ...
  >([]);
  const [latestDrivers, setLatestDrivers] = React.useState<
    {
      name: string;
      phone: string;
      status: "pending" | "approved" | "blocked";
  ...
  >([]);
  const [latestAdmins, setLatestAdmins] = React.useState<
    {
      name: string;
      phone: string;
      status: "pending" | "approved" | "blocked";
  ...
  React.useEffect(() => {
    Promise.all([
      ...
      adminApi.getMembers(token),
      adminApi.getDrivers(token),
      adminApi.getAdmins(token),
    ])
      .then(([s, members, drivers, admins]) => {
        setStats(s);
        const toRecent = (
          list: Array<{
            fullName: string;
        ...
        setLatestMembers(toRecent(members));
        setLatestDrivers(toRecent(drivers));
        setLatestAdmins(toRecent(admins));
      })
      .catch(console.error)
